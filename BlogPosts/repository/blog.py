...
def create_post(request:SchemasBlog,db:Session):
    new_blog = BlogPost(title=request.title,content=request.content,author=request.author)
    db.add(new_blog)
    db.commit()
    db.refresh(new_blog)
    return new_blog
...
def get_title(blog_title:str,db:Session):
    blog = db.query(BlogPost).filter(BlogPost.title == blog_title).first()
    if not blog:
...
def get_title_author(title:str,author:str,db:Session):
    blog = db.query(BlogPost).filter(BlogPost.title==title,BlogPost.author==author).first()
    if not blog:
